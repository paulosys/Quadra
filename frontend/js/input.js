...
export function setupInputListeners(send) {
  window.addEventListener('keydown', e => {
    ...
    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.code))
      e.preventDefault();
    unlockAudio();
    if (e.code === 'Space') { _tryKick(send); _tryPulse(send); }
  });
...
function _tryPulse(send) {
  if (state.gameState !== 'playing' || state.mySlot < 0) return;
  if ((Date.now() - state.pulseLastFired) / 1000 < PULSE_COOLDOWN_SECS) return;
